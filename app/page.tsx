...
import pricingCards from './libs/priceCard'
    ...
    <main className='flex min-h-screen h-fit flex-col items-center justify-center relative'>
      ...
      <header id="home" className="flex flex-col-reverse md:flex-row w-full h-screen max-w-7xl items-center justify-center p-8 relative overflow-x-hidden">
        <div className='w-full h-2/4 md:h-full md:w-2/5 flex flex-col justify-center items-center md:items-start gap-8'>
          <div className='flex flex-col gap-2'>
            <h1 className='text-4xl font-black md:text-8xl'>KODAKEY</h1>
            <h2 className='text-md md:text-2xl'>Start Scaling with Cloud!"</h2>
          </div>
          <p className='max-w-md text-sm md:text-base text-zinc-500'>Unleashing Compute Power, Robust Database Storage, Seamless Content Delivery, a Flexibility, Scalability, and Reliability.</p>
      ...
      <section id="pricing" className="h-fit min-h-screen w-full flex flex-col items-center justify-center gap-8 p-8">
        ...
        <div className='grid grid-cols-1 grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 items-center h-fit w-full max-w-3xl gap-8'>
            {pricingCards.map((pricingCard: { oneliner: string; title: string; price: number; benefits: string[]; id: Key | null | undefined }) => {
              return (
